...
def merge_documents(
    ...
    *,
        ...
        for row in sheet.rows:
            ...
            if _by_article(key) or not _GROUP_PREFIX.match(row.article):
                ...
                for k in _PACKING_OVERWRITE:
                    # packing wins over sender-spec aggregate
    ...
    for it in items.values():
        hit = catalog.get(it.key)
        if hit:
            _prefer(it, "customs_code", hit.get("customs_code"), overwrite=not it.fields.get("customs_code"))
            _prefer(it, "hs_code", hit.get("hs_code"))
            _prefer(it, "description", hit.get("description"))
